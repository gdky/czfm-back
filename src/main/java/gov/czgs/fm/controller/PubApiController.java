...
public class PubApiController {
	...
	public ResponseEntity<?> test() throws IOException {
		...
		outSTr = new FileOutputStream(new File("d:/add0.mp3"));   
        BufferedOutputStream Buff=null; 
		List<String> fdwz = getFdWz(sb);
		Buff=new BufferedOutputStream(outSTr);  
		for(String dw :fdwz){
			byte[] test = HttpUtil.http(dw);
			Buff.write(test);  
		}
         Buff.flush();   
         ...
         response.setContentType("application/pdf");
			...
			IOUtils.copyLarge(new FileInputStream("d:/add0.mp3"), response.getOutputStream());
			response.flushBuffer();
		return new ResponseEntity<>("d:/add0.mp3", HttpStatus.OK);
	}
